-- THIS IS JUST EXAMPLE CODE NOT USED IN THE ACTUAL PROGRAM
--
...
-- Useful functions created elsewhere
displayClock :: (Reflex t, MonadHold t m, MonadFix m, HasImageWriter t m, HasDisplayRegion t m, HasTheme t m) => Dynamic t (Maybe UTCTime) -> m ()
displayClock clockDyn = do
    let formatTime' = maybe "00:00:00" (T.pack . formatTime defaultTimeLocale "%H:%M:%S")
    text $ pure "TESTING CLOCK" -- Test if basic text works
    text $ pure $ formatTime' Nothing -- Test if formatting works
    let formattedTimeDyn = fmap formatTime' clockDyn
    text $ current formattedTimeDyn -- Original display


clockEvent :: (Reflex t, MonadHold t m, MonadFix m, PerformEvent t m, TriggerEvent t m, MonadIO (Performable m), PostBuild t m) => m (Event t (Maybe UTCTime -> Maybe UTCTime))
clockEvent = do
    tick <- tickLossyFromPostBuildTime 1
    newTime <- performEvent $ fmap (const $ liftIO getCurrentTime) tick
    return $ fmap (\t _ -> Just t) newTime
...
clockDyn <- foldDyn ($) (Just currentTime) =<< clockEvent
